...
class ConferenceUsersController extends Controller
{
    ...
    {       
       $conference=DB::select("SELECT DISTINCT email,first_name,last_name , referrer , product_detail_videos_id ,name , already_registerd FROM conference_users left join users on (users.id = users_id) 
